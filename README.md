...
Every number in this README comes from a notebook that was actually executed end-to-end on the machine described in [Setup](#setup) — nothing here is estimated or asserted without a corresponding run. The one exception to "recorded in the notebook itself" is the manual audit of notebook 08's RAG answers, which comes from a separate, clearly labeled re-run of the same pipeline (see [Key findings](#key-findings)).
...
- RAG demonstrated the gap between pretrained knowledge and organization-specific context directly. By the notebook's automated keyword scorer, the model matched the policy 0/9 times unaided and 6/9 times with retrieval, and retrieval itself never missed the right document (0/9 top-1 misses). A later manual audit of a fresh re-run (raw answers in [`results/08_rag_audit_raw_answers.json`](results/08_rag_audit_raw_answers.json)) refined that: of the 9 answerable questions, 7 RAG answers were correct as written and 2 were right in substance with a wording flaw (one turned the policy's "may be automatically isolated" into "will be"; one named the wrong actor for an escalation) — none was simply wrong, so "all nine fully correct" would overstate it. Unaided, the model was right on a couple of generic defaults, wrong on organization-specific facts (e.g., rating a developer laptop "high" criticality where the policy says Medium), and gave generic or hedged non-answers on the rest. RAG's real gain was on organization-specific facts, not general security knowledge.
- The failure that actually occurred was abstention: neither the unaided model nor RAG ever declined to answer a question with no supporting policy (0/3 correct refusals in both conditions, as recorded). With RAG, the model stated a 90-day firewall-log retention rule that appears in none of the policy documents.
- A separate failure, found only in the audit: citation faithfulness. With RAG, the model cited real policy documents for claims those documents do not contain — the 90-day retention rule cited to the incident response policy, and an invented VPN certificate process attributed to the Authentication Policy. That is different from retrieving the wrong document or giving no citation, and it makes an unsupported claim look traceable. The recorded run never scored citations.
...
6. **AI systems need observability across retrieval, models, tools, and outputs.** Notebook 08 Section 17's request trace (query → retrieval → scores → prompt → model version → response → latency → eval result) is what makes "why did the AI give the wrong answer" answerable after the fact instead of a guess. Citations need checking too: a cited source proves provenance, not support, and the audit of notebook 08 found real policy documents cited for claims they do not contain.
...
- Baseline (no retrieval): **0/9 correct** by the automated keyword scorer (6/12 labeled `hallucinated_policy` overall; that label means "answered without flagging insufficient information and did not match the reference," so it includes hedged, generic answers as well as confidently wrong ones). The manual audit found a mix: right on generic defaults (e.g., "No" to automated credential resets), wrong on organization-specific facts, and generic or hedged non-answers — so the strict 0/9 overstates how far behind the unaided model was on general knowledge.
- With retrieval: **6/9 correct** by the automated scorer (3/12 labeled `hallucinated_policy` overall, all on the unanswerable questions). The manual audit read the three `partially_correct` answers: two ("No" and "No, a service account may not authenticate interactively") were correct but terse, and one overstated "may" as "will"; it also found a wording flaw in one answer the scorer had marked correct. Net: 7 of 9 correct as written, 2 right in substance with a flaw, none wrong. Retrieval was never the bottleneck. Notebook 08 keeps its recorded outputs and scores unchanged and carries the corrections as later notes.
- **0/3 correct refusals** on unanswerable questions, in both the baseline and RAG conditions: neither configuration ever set the insufficient-information flag, even when instructed to and even when retrieved context didn't cover the topic. With RAG, one answer (Q11) acknowledged that no policy defined the escalation path and then answered anyway.
- **Citation faithfulness (audit only):** the model cited real policy documents for claims they do not contain (Q10, Q12; Q11 mixed) — a failure distinct from wrong retrieval or a missing citation, and one the recorded scorer never measured.
- **Audit evidence:** [`results/08_rag_audit_raw_answers.json`](results/08_rag_audit_raw_answers.json) holds all 24 raw answers, reference answers, scorer verdicts, retrieval results with similarity scores, citations, and the model and generation settings. It comes from a fresh audit run, not the original experiment (the original run saved only verdict labels), and the recorded notebook outputs are left as they were.
...
- Free-text answers are scored by transparent keyword-overlap heuristics, not human review. In notebook 08, a later manual audit compared the scorer's labels with a human reading of every answer and found them differing in both directions (terse-but-correct answers under-credited; a wrong actor and a "may" turned into "will" not flagged). The original run saved only verdict labels, so the audit re-ran the pipeline — treat it as a single-sample, single-reviewer review, not a replication.
...
│
├── results/                          # plots referenced above, plus 08_rag_audit_raw_answers.json (raw answers from a later audit run of notebook 08)
│
├── docs/
